...
public class EndpointExtraction {
    ...
    public static Set<Endpoint> extractEndpoints(Class<?> clazz, AnalysisMetaAccess metaAccess, Inflation bb, String msName) {
        ...
        try {
            ...
            // Obtaining the class path (which will be combined with the method's path for a full
            // URI)
                    ...
                    for (Annotation annotation : annotations) {
                        ...
                        if (controllerAnnotationNames.contains(annotation.annotationType().getSimpleName())) {
                            ...
                            try {
                                ...
                                // annotation.toString());
                                // path is optional, thus attempting to get it and return null if
                                // so.
                            ...
                            // Have to also consider the "path" JSON attribute of the annotation,
                            // (Example from train-tickets admin-user service):
                            // @org.springframework.web.bind.annotation.GetMapping(path={"/welcome"},
                            // headers={}, name="", produces={}, params={},
                            // value={}, consumes={})
                            // This is with the assumption that a value == path when within an
                            // annotation!
                            if (!hasPath) {
                                try {
                             ...
                             * produces = MediaType.APPLICATION_JSON_VALUE) public List<Question>
                             * getExamDetail(@PathVariable Integer id) {
                             */
                             ...
                             * produces = MediaType.APPLICATION_JSON_VALUE) public Exam
                             * submitExam(@PathVariable("id") Integer id) {
                             *
                             * NOTE: This is a second attempt at fetching the path! (if the first
                        ...
                        if (isEndpoint) {
                            ...
                            // add the controller path as well (if it has a RequestMapping
                            // annotation)
    ...
    public static String extractReturnType(AnalysisMethod method) {
        ...
        if (returnType instanceof ParameterizedType) {
            ...
            try {
                 ...
                 * @CrossOrigin
                 *
                 * @DeleteMapping("/{cateogryId}") public ResponseEntity<?>
                 * deleteCateogry(@PathVariable Long cateogryId)
                 */
